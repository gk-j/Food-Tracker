...
function BarChart({ data, goal, color }: { data: DayData[]; goal: number; color: string }) {
      ...
      >
        ...
        <View style={styles.toggle}>
          <Pressable
            ...
            onPress={() => { setView("weekly"); Haptics.selectionAsync(); }}
          >
          ...
          <Pressable
            ...
            onPress={() => { setView("monthly"); Haptics.selectionAsync(); }}
          >
            <Text style={[styles.toggleText, view === "monthly" && styles.toggleTextActive]}>
              Monthly
            </Text>
          </Pressable>
        </View>
        ...
        {view === "weekly" && (
          <View style={styles.weekNav}>
            ...
            <Pressable
              onPress={() => { if (weekOffset > 0) { setWeekOffset((o) => o - 1); Haptics.selectionAsync(); } }}
        ...
        <View style={styles.statsSection}>
          ...
          <View style={styles.statsGrid}>
            <StatCard label="Avg Calories" value={avgCal} unit=" kcal" color={Colors.dark.accent} />
            <StatCard label="Avg Protein" value={avgProt} unit="g" color={Colors.dark.protein} />
            <StatCard label="Avg Carbs" value={avgCarb} unit="g" color={Colors.dark.carbs} />
            <StatCard label="Avg Fats" value={avgFat} unit="g" color={Colors.dark.fats} />
          </View>
        </View>
...
const chartStyles = StyleSheet.create({
  container: { gap: 8 },
  chart: { flexDirection: "row", alignItems: "flex-end", gap: 4, height: 200, paddingBottom: 24 },
  barCol: { flex: 1, alignItems: "center", justifyContent: "flex-end" },
  barTrack: { justifyContent: "flex-end", alignItems: "center" },
  bar: { borderRadius: 4 },
  barLabel: { fontFamily: "Inter_400Regular", fontSize: 10, color: Colors.dark.textMuted, marginTop: 4 },
  goalLabel: { fontFamily: "Inter_400Regular", fontSize: 11, color: Colors.dark.textMuted, textAlign: "right" },
});
...
const statStyles = StyleSheet.create({
  ...
  label: { fontFamily: "Inter_400Regular", fontSize: 11, color: Colors.dark.textMuted, textAlign: "center" },
});

const styles = StyleSheet.create({
  container: { flex: 1 },
  content: { paddingHorizontal: 16, gap: 16 },
  screenTitle: { fontFamily: "Inter_700Bold", fontSize: 28, color: Colors.dark.text },
  toggle: {
    flexDirection: "row",
    backgroundColor: Colors.dark.card,
    borderRadius: 12,
  ...
  toggleText: { fontFamily: "Inter_600SemiBold", fontSize: 14, color: Colors.dark.textSecondary },
  ...
  chartTitle: { fontFamily: "Inter_600SemiBold", fontSize: 15, color: Colors.dark.text },
  chartEmpty: { height: 160, alignItems: "center", justifyContent: "center" },
  chartEmptyText: { fontFamily: "Inter_400Regular", fontSize: 14, color: Colors.dark.textMuted },
  statsSection: { gap: 10 },
  sectionTitle: { fontFamily: "Inter_700Bold", fontSize: 18, color: Colors.dark.text },
  sectionSub: { fontFamily: "Inter_400Regular", fontSize: 12, color: Colors.dark.textMuted, marginTop: -6 },
  ...
  macroLegend: { flexDirection: "row", justifyContent: "center", gap: 16 },
  macroLegendItem: { flexDirection: "row", alignItems: "center", gap: 6 },
  macroLegendDot: { width: 8, height: 8, borderRadius: 4 },
  macroLegendText: { fontFamily: "Inter_400Regular", fontSize: 12, color: Colors.dark.textSecondary },
});
